...
//Move	
		...
		if(DEPTH_RECEIVED){//==!(depthimg->image.empty())
			depth_img=depthimg->image.clone();
		}
		if(PreLimg.empty()||!DEPTH_RECEIVED||!ODOMETRY_RECEIVED/*||PreRimg.empty()*/){	//差分を取るための処理(ループ１回目のみ処理をしない)
		}										
		else{
...
//msg for publisher
			...
			//			std::cout<<"pixcel:("<<RectPoint_tl[i].x+(int)points[i][j].x<<","<<RectPoint_tl[i].y+(int)points[i][j].y<<")\t"<<"z:"<<z<<'\n';
					}
...
//only x
/*						float opticalflow_size_prev=sqrt(
							newpoints[i][j].x-points[i][j].x
							);	
						
						float opticalflow_size=sqrt(
							(newpoints[i][j].x-points[i][j].x-value_x)
							);
*/
//so far			
//						std::cout<<"flow_size:"<<"prev:"<<opticalflow_size_prev
//								<<"\tcurrent:"<<opticalflow_size<<'i\n';
// Variation of odametry is 2
//Search Variation Program
/*						if(opticalflow_size_prev-opticalflow_size>2){
							ROS_INFO("flow Offset!::point(%d,%d)",
									(int)(RectPoint_tl[i].x+points[i][j].x),
									(int)(RectPoint_tl[i].y+points[i][j].y)); 
							if(max_value_x<value_x)
								max_value_x=value_x;
							if(max_value_y<value_y)
								max_value_y=value_y;
							if(min_value_x>value_x)
								min_value_x=value_x;
							if(min_value_y>value_y)
								min_value_y=value_y;
							ROS_INFO("value(x,y):(%f,%f)",value_x,value_y);
							ROS_INFO("max_value(x,y):(%f,%f)",max_value_x,max_value_y);
							ROS_INFO("min_value(x,y):(%f,%f)",min_value_x,min_value_y);
						}
*/
//convert to value of degital 
//						int dvalue=5;
//						opticalflow_size_prev=(int)(opticalflow_size_prev*10)/dvalue*dvalue/10.0;
//						opticalflow_size=(int)(opticalflow_size*10)/dvalue*dvalue/10.0;
//So far
//debug
                            printf("v(%f),w(%f),dt(%f),flow(%f,%f),value(%f,%f)\n",
								v,w,dt,
                                newpoints[i][j].x-points[i][j].x,
                                newpoints[i][j].y-points[i][j].y,
                                value_x,value_y); 
//
						float th_val=1.0;
						if((value_x>th_val||value_y>th_val||value_x<-th_val||value_y<-th_val)&&
							...
							opticalflow_size<5){
//Debug program
//							ROS_INFO("(opticalflow_size):(%f)",
//								opticalflow_size
//								);
//							ROS_INFO("(%f,%f)-point:(%d,%d)",
//								opticalflow_size_prev,
//								opticalflow_size,
//								RectPoint_tl[i].x+(int)points[i][j].x,
//								RectPoint_tl[i].y+(int)points[i][j].y);
...
//	std::cout<<"Pre_time:"<<delta_t_sec0<<'\n';
...
//	std::cout<<"minfps:"<<minfps<<'\n';	
//	std::cout<<"ImageProcess--fps:"<<fps<<'\n';
//--------------------------------------
}
