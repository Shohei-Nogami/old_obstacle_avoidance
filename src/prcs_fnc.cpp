...
//						mk_temp.release();
						xt_once_p.push_back(0);
						double qt_data[3][3]={ {0.05,0,0},{0,0.05,0},{0,0,0.05} };
//						cv::Mat qt_temp(3,3,CV_64FC1,qt_data);
						qt_p.push_back(cv::Mat(3,3,CV_64FC1,qt_data));
						double xt_dif_ave_data[3][1]={ {0},{0},{0} };
						xt_dif_ave_p.push_back(cv::Mat(3,1,CV_64FC1,xt_dif_ave_data));
...
//				}
			...
			qt.push_back(qt_p[i]);
			xt_dif_ave.push_back(xt_dif_ave_p[i]);
...
//		double rt_data[3][3]={ {0.05,0,0},{0,0.05,0},{0,0,0.05} };
		double rt_data[3][3]={ {0.0025,0,0},{0,0.0025,0},{0,0,0.0025} };
...
//--		
		double xt_0_data[3][1]={ {(points[j].x-width/2)*z[j]/f},{-(points[j].y-height/2)*z[j]/f},{z[j]} };
		cv::Mat xt_0(3,1,CV_64FC1,xt_0_data);
		xt_hat[j]=xt_0.clone();		
//--
		if(xt_hat[j].at<double>(0,0)==0||xt_once[j]==0){
//			std::cout<<"p,p_from_cp"<<points[j].y<<","<<-(points[j].y-height/2)<<"\n";
			double xt_0_data[3][1]={ {(points[j].x-width/2)*z[j]/f},{-(points[j].y-height/2)*z[j]/f},{z[j]} };
			cv::Mat xt_0(3,1,CV_64FC1,xt_0_data);
			xt_hat[j]=xt_0.clone();
//			double qt_data[3][3]={ {0.05,0,0},{0,0.05,0},{0,0,0.05} };
//			double qt_data[3][3]={ {0.0025,0,0},{0,0.0025,0},{0,0,0.0025} };
			double qt_data[3][3]={ {0.01,0,0},{0,0.01,0},{0,0,0.01} };
			cv::Mat qt_temp(3,3,CV_64FC1,qt_data);
			qt[j]=qt_temp.clone();
			xt_dif_ave[j]=xt_0.clone();
		}		
		xt_tld=at*xt_hat[j]+btut;
		sgm_tld=at*sgm[j]*at.t()+rt;
		
		cv::Mat xt_dif_temp=zt-xt_tld;
		cv::Mat xt_dif_ave_temp=(xt_dif_temp+xt_dif_ave[j])/2.0;
		double qt0_data[3][3]={ {0.0025,0,0},{0,0.0025,0},{0,0,0.0025} };
		cv::Mat qt0_temp(3,3,CV_64FC1,qt0_data);
//		cv::Mat qt_temp=qt0_temp+( (xt_dif_ave[j]-xt_dif_ave_temp)*(xt_dif_ave[j]-xt_dif_ave_temp).t()+
//						(xt_dif_temp-xt_dif_ave_temp)*(xt_dif_temp-xt_dif_ave_temp).t() )/2;

		cv::Mat qt_temp=qt0_temp+xt_dif_temp*xt_dif_temp.t();
//		cv::Mat qt_temp=qt[j]+xt_dif_temp*xt_dif_temp.t();
		qt[j]=qt_temp.clone();
		xt_dif_ave[j]=xt_dif_ave_temp.clone();
//		std::cout<<"xt_dif_ave["<<j<<"]:"<<xt_dif_ave[j]<<"\n";
		kt=sgm_tld*((sgm_tld+qt[j]).inv());
...
//		std::cout<<"x_hat[j]:"<<x_hat[j]<<"\n";
		...
		double th_mvobj=1;
		if(std::abs(dyaw)>0.008)
			th_mvobj=std::abs(dyaw)/0.008*0.90;
//			th_mvobj=std::abs(dyaw)*std::abs(dyaw)/(0.001*0.001);
//		if(sgm[j].at<double>(0,0)/*+sgm[j].at<double>(2,2)*/>0.001555){//*th_mvobj){//0.002){
		if(xt_hat[j].at<double>(1,0)+0.23>0&&xt_hat[j].at<double>(1,0)+0.23<1.0){
			if(sgm[j].at<double>(0,0)/*+sgm[j].at<double>(2,2)*/>0.00160*th_mvobj||xt_once[j]<10||sgm[j].at<double>(2,0)>0.005||sgm[j].at<double>(1,0)>0.002){//0.002){
	//		if(sgm[j].at<double>(0,0)/*+sgm[j].at<double>(2,2)*/>0.002*0.002){//*th_mvobj){//0.002){
						ImageProcesser::cvArrow(&Limg_view,
					cv::Point((int)points[j].x,
						(int)points[j].y),
